...
#let linear_trend(data) = {
    ...
    let m = data.map(e => (e.first()-mu_x)*(e.last()-mu_y)).sum() / data.map(e => calc.pow((e.first()-mu_x), 2)).sum()
...
#let horizontal_table(data, header, font-size:13pt) = {
  ...
  table(
    fill: (x, _) => if x == 0 { gray.lighten(75%) },
...
#let scatter(data, label: $y(x)$, x-label: $x$, y-label:$y$, size: (10, 8), legend: "legend.inner-east", x-tick-step: auto,y-tick-step: auto, color: blue, ..plots) = {
  canvas({
    import draw: *
    set-style(
      legend: (
...
#let scatter_with_trend(data, label: $y(x)$, x-label: $x$, y-label:$y$, size: (10, 8), legend: "legend.inner-east",x-tick-step: auto, y-tick-step: auto) = {
